...
namespace plot_roc_pr
{
    public class Program
    {
        ...
        {
            //var scaled = minScale + (float)(value - min) / (max - min) * (maxScale - minScale);

            return (maxScale - minScale) * ((value - min) / (max - min)) + minScale;
            //return scaled;
        }
...
";
            ...
            plot(perf_curve_types.roc, encoded_roc, $@"c:\svm_compute\charts\{nameof(perf_curve_types.roc)}.bmp");
            plot(perf_curve_types.pr, encoded_pr, $@"c:\svm_compute\charts\{nameof(perf_curve_types.pr)}.bmp");
            plot(perf_curve_types.pri_from_pr, encoded_pr, $@"c:\svm_compute\charts\{nameof(perf_curve_types.pri_from_pr)}.bmp");
        }

        public enum perf_curve_types
        {
            pr,
            pri,
            pri_from_pr,
        ...
        public static void plot(perf_curve_types perf_curve_type, string encoded_plot_data, string save_filename)
        {
          
            
            List<(string x_title, string y_title, List<(double x, double y)> xy)> parsed_xy/*roc*/ = encoded_plot_data.Split(new char[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
                .Select(line =>
                {
            ...
            //List<(string x_title, string y_title, List<(double x, double y)> xy)> parsed_pr = encoded_plot_data.Split(new char[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries)
            //    .Select(line =>
            //    {
            //        var headers = line.Trim().Split('/').First().Split(';');

            //        return (x_title: headers[0], y_title: headers[1], xy: line.Trim().Split('/').Skip(1).Select(b => (x: double.Parse("" + b.Split(';')[0]), y: double.Parse("" + b.Split(';')[1]))).ToList());
            //    }).ToList();

            //if (perf_curve_type == perf_curve_types.pr || perf_curve_type == perf_curve_types.pri || perf_curve_type == perf_curve_types.pri_from_pr)
            //{
            //    parsed_xy.Add(("x", "y", parsed_xy.SelectMany(a => a.xy).OrderBy(a => a.x).ThenBy(a => a.y).ToList()));
            //    parsed_xy = parsed_xy.Skip(parsed_xy.Count - 1).ToList();
            //}
            ...
            // for all x,y coordinates, find the closest x,y point in every outer-cv curve, then the averages will be taken

            var average_for_closest_points = true;
            var average_for_last_point = false;
            //var average_make_faux_x = true;

            (string x_title, string y_title, List<(double x, double y)> xy) average_xy = (parsed_xy.First().x_title, parsed_xy.First().y_title, new List<(double x, double y)>());

            //if (average_make_faux_x)
            //{
            //    var all_x = parsed_xy.SelectMany(b => b.xy.Select(c => c.x).Distinct().ToList()).Distinct().ToList();

            //    foreach (var a in parsed_xy)
            //    {
            //        foreach (var faux_x in all_x)
            //        {
            //            var faux_y = ;
            //            a.xy.Add((faux_x,faux_y));
            //        }
            ...
            if (average_for_closest_points)
            {
                ...
                average_xy.xy = closest_to_all_xy.Select(a => (x: a.Average(b => b.x), y: a.Average(b => b.y))).Distinct().OrderBy(a => a.x).ThenBy(a => perf_curve_type == perf_curve_types.roc ? a.y : 1 - a.y).ToList();

                average_xy.xy = average_xy.xy.Distinct().OrderBy(a => a.x).ThenBy(a => perf_curve_type == perf_curve_types.roc ? a.y : 1 - a.y).ToList();


                //average_xy.xy = average_xy.xy.Select(a => (a.x, a.y - 0.01)).ToList();
            }

            if (average_for_last_point)
            {
                var all_xy = parsed_xy.SelectMany(a => a.xy).OrderBy(a => a.x).ThenBy(a => perf_curve_type == perf_curve_types.roc ? a.y : 1 - a.y).Distinct().ToList();

                average_xy.xy.Clear();
                
                foreach (var point_of_interest in all_xy)
                {
                    
                    // get the points either side of x,y

                    var xy_before = parsed_xy.Select(b => b.xy.OrderBy(c => c.x).ThenBy(c => perf_curve_type == perf_curve_types.roc ? c.y : 1 - c.y).FirstOrDefault(c => c.x <= point_of_interest.x && c.y <= point_of_interest.y)).ToList();
                    var xy_after = parsed_xy.Select(b => b.xy.OrderBy(c => c.x).ThenBy(c => perf_curve_type == perf_curve_types.roc ? c.y : 1 - c.y).FirstOrDefault(c => c.x >= point_of_interest.x && c.y >= point_of_interest.y)).ToList();
                    var xy_middle = xy_before.Select((a, i) =>
                        {
                            var v_x = 0d;
                            var v_y = 0d;

                            var before_x = xy_before[i].x;
                            var after_x = xy_after[i].x;
                            ...
                            {
                                var p_x = point_of_interest.x;

                                var s_x = scale(p_x, before_x, after_x, 0, 1);

                                var d_x = (after_x - before_x) * s_x;

                                v_x = before_x + d_x;
                            }

                            var before_y = xy_before[i].y;
                            var after_y = xy_after[i].y;
                            ...
                            {
                                var p_y = point_of_interest.y;

                                var s_y = scale(p_y, before_y, after_y, 0, 1);

                                var d_y = (after_y - before_y) * s_y;
                    ...
                    //xy_middle = xy_middle.OrderBy(c => c.x).ThenBy(c => perf_curve_type == perf_curve_types.roc ? c.y : 1 - c.y).ToList();

                    average_xy.xy.Add((xy_middle.Average(a => a.x), xy_middle.Average(a => a.y)));

                    //List<(double x, double y)> xy_until_now =null;

                    //if (perf_curve_type == perf_curve_types.roc)
                    //{
                    //    xy_until_now = parsed_xy.Select(b => b.xy.OrderBy(a => a.x).ThenBy(a => perf_curve_type == perf_curve_types.roc ? a.y : 1 - a.y).DefaultIfEmpty((x: 0, y: 0)).FirstOrDefault(c => c.x >= point_of_interest.x && c.y <= point_of_interest.y)).ToList();
                    //}
                    //else
                    //{
                    //    xy_until_now = parsed_xy.Select(b => b.xy.OrderBy(a => a.x).ThenBy(a => perf_curve_type == perf_curve_types.roc ? a.y : 1 - a.y).DefaultIfEmpty((x: 0, y: 0)).FirstOrDefault(c => c.x >= point_of_interest.x && c.y <= point_of_interest.y)).ToList();
                    ...
                    //xy_until_now = xy_until_now.Where(a => a.x > 0 || a.y > 0).ToList();

                    //if (xy_until_now == null || xy_until_now.Count == 0)
                    //{
                    //    xy_until_now.Add((0,0));
                    //}

                    //var xy_until_now_average = (xy_until_now.Min(a => a.x), xy_until_now.Min(a => a.y));
                    //average_xy.xy.Add(xy_until_now_average);
                }
            ...
            //if (perf_curve_type == perf_curve_types.pr || perf_curve_type == perf_curve_types.pri || perf_curve_type == perf_curve_types.pri_from_pr)
            //{
            //    average_xy.xy = average_xy.xy.OrderBy(a => a.y).ThenBy(a => a.x).ToList();
            //}



            //parsed_roc.Add(parsed_roc.SelectMany(a => a).OrderBy(a => a.x).ThenBy(a => a.y).ToList());

            //parsed_roc = parsed_roc.Skip(parsed_roc.Count - 1).ToList();


            //var data = parsed_pri;
            //data.AddRange(parsed_pr);
            ...
            var graph_width_x = (double) bmp.Width * 0.8;
            var graph_height_y = (double) bmp.Height * 0.8;
            ...
            Console.WriteLine($@"{nameof(graph_start_x)}={graph_start_x}, {nameof(graph_start_y)}={graph_start_y}, {nameof(graph_end_x)}={graph_end_x}, {nameof(graph_end_y)}={graph_end_y}");
            ...
            using (var bmp_gfx = Graphics.FromImage(bmp))
            {
                ...
                // draw grid lines horizonal (x changes, y in min to max)
                {
                    ...
                    for (var x = 0.0; x <= 1.0; x += 0.1)
                    {
                        ...
                        Console.WriteLine($@"{nameof(x1)}={x1}, {nameof(y1)}={y1}, {nameof(x2)}={x2}, {nameof(y2)}={y2}");
                ...
                {
                    var is_average_curve = i == data.Count - 1;

                    var c = Color.FromArgb(color_rnd.Next(0, 255), color_rnd.Next(0, 255), color_rnd.Next(0, 255));
            ...
            //var save_file = @"c:\bioinf\image.bmp";
